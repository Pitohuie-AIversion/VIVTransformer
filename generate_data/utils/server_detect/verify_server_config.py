...
class ServerConfigValidator:
    ...
    def generate_report(self) -> str:
        ...
        report = []
        report.append("="*60)
        report.append("[REPORT] 服务器配置验证报告")
        report.append("="*60)
        ...
        if self.errors:
            report.append("[ERROR] 验证失败，请修复上述错误后重试")
        elif self.warnings:
            report.append("[WARN] 验证通过但有警告，建议检查警告项目")
        else:
            report.append("[OK] 验证完全通过，可以开始训练！")
