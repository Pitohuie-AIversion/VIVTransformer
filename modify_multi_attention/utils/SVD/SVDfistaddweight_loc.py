...
def map_to_coordinates(weighted_first_mode, difference_matrix):
    ...
    :return: 映射后的坐标值（返回一组坐标）
    """
    # 假设加权后的特征向量将映射到差异矩阵的位置上
    coords = np.dot(difference_matrix, weighted_first_mode)  # 将加权模态与差异矩阵相乘，得到坐标值
    # 获取差异矩阵的尺寸
    height, width = difference_matrix.shape
    # 返回每个坐标点的 (x, y) 坐标
    return [(i % width, i // width) for i in range(len(coords)) if coords[i] > np.mean(coords)]  # 选择加权值较大的坐标点
...
def plot_difference_figure(difference_matrix, time_step, epoch, attention_type, idx, parent_dir="attention_results",
                           mode="test", high_energy_region=None, weighted_coords=None, radius=5):
    """
    绘制差异图并用圆形框选加权模态的位置。展示流场的差异。
    :param difference_matrix: 差异矩阵
    ...
    :param weighted_coords: 加权模态对应的坐标
    ...
    if weighted_coords is not None:
        for coord in weighted_coords:
            plt.scatter(coord[0], coord[1], color='blue', marker='x', label='Weighted Mode Position')
...
def svd_analysis(file_path, threshold=0.9, output_dir="svd_results", top_n=10, weight_factor=1.0):
    ...
    # 将加权后的模态1映射到坐标上
    weighted_coords = map_to_coordinates(weighted_first_mode, difference_matrix)
