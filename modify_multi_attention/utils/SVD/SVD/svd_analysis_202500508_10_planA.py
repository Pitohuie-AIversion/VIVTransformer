...
def batch_process_svd(input_dir, output_dir, threshold=0.9,
    ...
    matched = [pattern.match(os.path.basename(f)) for f in files]
    matched = [m for m in matched if m]
    if not matched:
        raise RuntimeError(
            "No files match pattern "
            "Re_{re}_time_{time}_train_epoch_{it}_sample_{sample}_difference_matrix.npy"
        )

    re_list   = sorted({int(m.group('re'))   for m in matched})
    t_list    = sorted({float(m.group('time')) for m in matched})
    iter_list = sorted({int(m.group('it'))   for m in matched})
    ...
    plot_similarity_matrix(sim_all, files,
...
if __name__ == "__main__":
    batch_process_svd(
        ...
        top_n=10,
        weight_factor=1.5,
        num_modes=10  # 提取前10个主成分模式，并按方案A保留每次迭代
    )
