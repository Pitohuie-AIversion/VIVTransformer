...
def test_cpu_memory_monitoring():
    ...
    try:
        ...
        logger.info("✅ CPU内存监控和优化功能正常")
        return True
        
    except Exception as e:
        logger.error(f"❌ CPU内存监控测试失败: {e}")
        return False
...
def test_dataloader_optimization():
    ...
    try:
        ...
        if num_workers == 0 and cpu_count > 4:
            ...
            logger.info(f"🚀 自动优化: num_workers从{num_workers}调整为{optimized_workers}")
        
        logger.info("✅ 数据加载器优化逻辑正常")
        return True
        
    except Exception as e:
        logger.error(f"❌ 数据加载器优化测试失败: {e}")
        return False
...
def test_tensor_optimization():
    ...
    try:
        ...
        logger.info(f"✅ Tensor优化创建成功: dtype={tensor.dtype}, contiguous={tensor.is_contiguous()}")
        return True
        
    except Exception as e:
        logger.error(f"❌ Tensor优化测试失败: {e}")
        return False
        ...
        if config_path.exists():
            ...
            for key in expected_keys:
                if key in dataloader_config:
                    logger.info(f"✅ 配置项 {key}: {dataloader_config[key]}")
                else:
...
def test_system_resources():
    ...
    try:
        ...
        logger.info(f"💾 可用内存: {memory_available_gb:.1f}GB")
        logger.info(f"🚀 GPU可用: {gpu_available}")
        logger.info(f"🚀 GPU数量: {gpu_count}")
        
        # 服务器配置建议
        if cpu_count >= 16:
            logger.info("🎯 检测到高性能CPU，建议使用多进程数据加载")
        if memory_gb >= 32:
            logger.info("🎯 检测到大内存，建议启用数据预加载")
        if gpu_count > 1:
            logger.info("🎯 检测到多GPU，建议使用DataParallel")
        
        logger.info("✅ 系统资源检测完成")
        return True
