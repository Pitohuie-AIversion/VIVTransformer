...
def test_training_pipeline():
    ...
    try:
        ...
        try:
            ...
            print("  ✅ dynamic_resolution_trainer 导入成功")
        except Exception as e:
            print(f"  ❌ dynamic_resolution_trainer 导入失败: {e}")
            print("  ℹ️ 这可能是由于依赖模块路径问题，但不影响配置测试")
            trainer = None
        ...
        if trainer is not None:
            ...
            print("  ⚠️ 跳过函数检查（模块导入失败）")
        
        # 4. 数据路径验证
        print("\n📁 数据路径验证:")
        data_path = config['data']['path']
        if os.path.exists(data_path):
            print(f"  ✅ 数据文件存在: {os.path.basename(data_path)}")
        ...
        print(f"  可行性评分: {max(0, feasibility_score)}/100")
        
        if feasibility_score >= 80:
            print("  ✅ 训练配置良好，可以开始训练")
        elif feasibility_score >= 60:
            print("  ⚠️ 训练配置基本可行，建议优化")
        else:
            print("  ❌ 训练配置存在问题，建议修复后再训练")
